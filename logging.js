...
async function logResourceAccess(url, ipAddress)
{
    try
    {
        ...
        let table = 'resource_access_logs';
        const result = await sql`
        insert into ${sql(table)} ${
            sql(resourceAccessRecord, 'timestamp', 'location', 'ip_address')
        }
        `;
        // Log the result of the SQL command
        // appendToLog('cjremmett_logs', 'LOGGING', 'DEBUG', `logResourceAccess SQL result: ${JSON.stringify(result)}`);
    }
    catch(err)
    {
